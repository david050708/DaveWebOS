...
login_screen = Tk()
login_screen.title("Login to DaveWebOS")
login_screen.attributes("-fullscreen", True)

usernamelabel = ttk.Label(login_screen, text="Username").pack()
username = ttk.Entry(login_screen).pack()

passwordlabel = ttk.Label(login_screen, text="Password").pack()
password = Entry(login_screen).pack()
...
def theos():
    ...
    oswindows.title("DaveOS")

    def notepad():
        app = Tk()
        app.title("DaveOS Text Editor")
    ...
    file_explorerlabel.place(x=488, y=130)
...
def go(event=None):
    ...
    theos()
login_screen.bind("<Return>", go)

gobutton = Button(login_screen, text="-->", command=go)
gobutton.pack()

def lazy():
    global lazytotypebt
    lazytotypebt.pack_forget()
    label = Label(login_screen, text="Just press the button enter...")
    label.pack()

lazytotypebt = Button(login_screen, text="Forgot Password", command=lazy)
lazytotypebt.pack()
